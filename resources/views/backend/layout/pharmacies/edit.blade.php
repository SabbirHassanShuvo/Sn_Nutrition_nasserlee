        ...
        <div class="col-lg-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white py-3 d-flex align-items-center justify-content-between">
                    <div>
                        <h5 class="card-title mb-0 fw-bold text-primary">Edit Pharmacy</h5>
                        <p class="text-muted mb-0 fs-12">Update pharmacy details, address, phone number, coordinates, services, and photo</p>
                    </div>
                ...
                <div class="card-body">
                    <form action="{{ route('backend.pharmacies.update', $pharmacy->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.1" min="1" max="5" name="rating" class="form-control" value="{{ old('rating', $pharmacy->rating) }}">
                            </div>

                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold">Opening Hours</label>
                                <input type="text" name="opening_hours" class="form-control" value="{{ old('opening_hours', $pharmacy->opening_hours) }}">
                            </div>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="text" name="address" id="pharmacyAddressInput" class="form-control @error('address') is-invalid @enderror" value="{{ old('address', $pharmacy->address) }}" required>
                                ...
                                <div class="card border bg-light p-3">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" name="latitude" id="latInput" class="form-control form-control-sm bg-white" value="{{ old('latitude', $pharmacy->latitude) }}">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label fs-12 fw-medium text-muted">Longitude</label>
                                            <input type="text" name="longitude" id="lngInput" class="form-control form-control-sm bg-white" value="{{ old('longitude', $pharmacy->longitude) }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12 mb-4">
                                ...
                                <div class="d-flex flex-wrap gap-2 p-3 bg-light rounded border" id="servicesContainer">
                                    @php
                                        $currentServices = $pharmacy->services ?? [];
                                    @endphp
                                    @foreach($currentServices as $sIdx => $service)
                                        <div class="service-pill-wrapper me-1 mb-1">
                                            <input type="checkbox" name="services[]" value="{{ $service }}" id="service_{{ $sIdx }}" class="service-pill-checkbox" checked>
                                ...
                                <small class="text-muted mt-1 d-block">Click <strong>&times;</strong> to remove any service tag.</small>
                            </div>

                            <!-- Photo / Image Upload (Dropify) -->
                            <div class="col-md-12 mb-4">
                                <label class="form-label fw-semibold">Pharmacy Photo / Logo</label>
                                <input type="file" name="image" class="dropify" data-height="160" data-max-file-size="20M" accept="image/*" @if($pharmacy->image) data-default-file="{{ asset($pharmacy->image) }}" @endif />
                            </div>

                            <div class="col-md-12 mb-3">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="is_active" value="1" id="is_active" {{ $pharmacy->is_active ? 'checked' : '' }}>
                        ...
                        <div class="text-end">
                            <button type="submit" class="btn btn-primary px-4"><i class="ri-save-line me-1"></i> Update Pharmacy</button>
                        </div>
                    </form>
                </div>
            </div>
...
@push('scripts-bottom')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var map = L.map('pharmacyMapPreview').setView([initialLat, initialLng], 14);
